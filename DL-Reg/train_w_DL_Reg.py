...
def main():
    ...
    parser.add_argument('--gamma', type=float, default=2e7)
    ...
    ax_acc.legend()
    fig_loss.savefig(config['experiment_name']+"_loss.jpg",dpi=600)
    fig_acc.savefig(config['experiment_name']+"_acc.jpg",dpi=600)
    torch.save(metric,config['experiment_name']+"_metric.pth")
    if config['show_test']:
